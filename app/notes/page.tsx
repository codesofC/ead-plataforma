...
const Notes = () => {
  ...
  return (
    <MyLayout>
      <main className="w-full flex flex-col gap-6 px-2 md:px-6 lg:px-12 my-16">
                ...
                >
                  ...
                  <AccordionContent className="flex flex-col gap-6 px-2 md:px-6">
                    {notes.semestre.map((noteCourse: any, _i: number) => (
                      <div key={_i} className="flex gap-2 justify-between items-center">
                        <span className="font-semibold">{ noteCourse.module}</span>
                        <span className="p-2 shadow-inner bg-white font-bold rounded-sm border">{ noteCourse.note }</span>
                      </div>
                    ))}
                  </AccordionContent>
                </AccordionItem>
